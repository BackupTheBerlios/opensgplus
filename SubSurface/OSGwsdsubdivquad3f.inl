 ...
 OSG::Int32 crease)
{
   if ((crease == 0) || 
      ...
      v= (v1 + v2)*0.375f + (w1 + w2 + w3 + w4)*0.0625f;
   } else {
      v= (v1 + v2)*0.5f;
   }
}
...
{   
   ...
   if (abs(b-a) < 3) {    
      t1 = (v0 * 2.0f) + ((array[a] + array[b%(val*2)]) * -1.0f);    
      v = t2.cross(t1);
   } else {
...
void WSDsubdiv<OSG::Vec3f, QUAD>::firstlimitrow (OSG::UInt32 &lwi)
{
   ...
   OSG::Int32 i=1;
   OSG::UInt32 nwi=lwi;
   if (ppatch->iscorner[0] > 2) {        // corner, so just copy
      OSG::Vec3f h = tabA[ri*bA+i];        
...
{     
   ...
   OSG::Int32 i=1;
   OSG::UInt32 nwi=lwi;
   if (ppatch->iscorner[1] > 2) {      // corner just copy
      OSG::Vec3f h = tabA[ri*bA+i];        
      ppatch->mySharedFields->limitpoints->setValue(Pnt3f(h.x(),h.y(),h.z()), lwi);      
      lwi+=step;    
