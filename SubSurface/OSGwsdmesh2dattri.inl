...
(HalfedgeHandle fhe_h)
{
   ...
   if (useCreases && (mesh->property(isCrease,mesh->edge_handle(test)))>0) {
...
(Int32 i, HalfedgeHandle &fhe_h, HalfedgeHandle &op)
{
   ...
   if (op == test) ppatch->valenz[i] = vt;            // corner!
   else {
      ende = op;          
      ...
      while (test != ende) {      
         ...
         // store creases         
         Int32 thiscrease = 0;
         if (useCreases) thiscrease = mesh->property(isCrease,mesh->edge_handle(test));
...
(Int32 i, Int32 a2, Int32 a7, HalfedgeHandle &fhe_h)      
{
   ...
   Int32 thiscrease = 0;
   if (useCreases) {
      thiscrease = mesh->property(isCrease,mesh->edge_handle(mesh->next_halfedge_handle(op)));
   }
   (ppatch->crease[a2]) = getCreaseType_Edge(    
    thiscrease,
    n2_regular,v0_regular);
   if (useCreases) {
      thiscrease = mesh->property(isCrease,mesh->edge_handle(
...
(Int32 i, Int32 a11, Int32 a10, HalfedgeHandle &fhe_h)      
{
   ...
   Int32 thiscrease = 0;
   if (useCreases) thiscrease = mesh->property(isCrease,mesh->edge_handle(mesh->prev_halfedge_handle(op)));
...
template<>
void WSDmesh2dat<OSG::Vec3f, MyTriMesh, TRIANGLE>::put_p_to
...
//*******  
   ...
   // creases
   if (useCreases) {
      ppatch->crease[5] = mesh->property(isCrease,mesh->edge_handle(fhe_h));//mesh->edge(mesh->edge_handle(fhe_h)).isCrease;
   } else {
      ppatch->crease[5] = 0;
   }
   // tex
...
//*************************************************************************************
   ...
   if (ppatch->tricrease[4] == 0)
      ppatch->tricrease[4] = ppatch->corcrease[2][1];   
   if (ppatch->tricrease[4] != 0) 
      ppatch->tricrease[4]=1;           // just mark as crease, details in subdiv
   ...
   ppatch->crease[7]  = ppatch->corcrease[1][3%ppatch->valenz[1]];    
   ppatch->crease[9]  = ppatch->corcrease[2][ppatch->valenz[2]-1];
   ppatch->crease[10] = ppatch->corcrease[1][ppatch->valenz[1]-2];
   ...
   for (Int32 ll=0; ll<4; ll++) {                           // all corners
      ...
      if (ppatch->iscorner[ll] >= 2) {         
         ...
         Int32 aa=vv;        
         Int32 bb=1;            
      if (ll == 2) {       // solve problem for single triangle
         aa++;
         bb++;
      }
