...
WSDmesh2dat<WSDVector, Mesh, mtype>::WSDmesh2dat(Mesh *m)
{
   mesh=m;
}
...
::collectCorner(Int32 i, HalfedgeHandle &fhe_h, HalfedgeHandle &op)
{
	...
	if (op == test) ppatch->valenz[i] = 2;		// ecke!
	else {
		ende = op;
...
template<class WSDVector, class Mesh, int mtype>
void WSDmesh2dat<WSDVector, Mesh, mtype>
::helper1(Int32 i, Int32 a2, Int32 a7, typename Mesh::HalfedgeHandle &fhe_h)			
...
::p_nach(const typename Mesh::Point& p, Int32 x, Int32 y) 
{
	(ppatch->tableO[x][y]).setValues(p[0], p[1], p[2], 1.0);
}

// insertface: main method for reading from openmesh
template<class WSDVector, class Mesh, int mtype>
void WSDmesh2dat<WSDVector, Mesh, mtype>
::insertface(typename Mesh::FaceHandle f_h, WSDdat<WSDVector, mtype> *pp)
{	
	ppatch = pp;
	OMPoint p;	
	Int32 vt;
...
//*************************************************************************************
	...
	//ppatch->radiusD
   WSDVector d1 = ppatch->tableO[1][1] - ppatch->tableO[2][2];
   WSDVector d2 = ppatch->tableO[1][2] - ppatch->tableO[2][1];
   ...
   ppatch->faceloc = (ppatch->tableO[1][1] + ppatch->tableO[2][1] +
					  ppatch->tableO[2][2] + ppatch->tableO[1][2]) * 0.25;
...
::getBoundingBox(OSG::Pnt3f& boundingMin, OSG::Pnt3f& boundingMax)
{
	...
	for (typename Mesh::VertexIter v_it=mesh->vertices_begin(); v_it!=mesh->vertices_end(); ++v_it) {
