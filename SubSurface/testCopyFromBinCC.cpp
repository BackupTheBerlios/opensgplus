...
#include <OSGSubdivisionCollector.h>
...
// to collect all the Subdivision Object in the scene
typedef OSGSubdivisionCollector<MyDynamicSubdivisionCCPtr> Collector;
Collector* MyCollector;

// dummy (otherwise its not recognized...)
...
int main(int argc, char **argv)
{
   ...
   std::ifstream is("binfileCC.osb", std::ios::binary);
   NodePtr scene = OSG::SceneFileHandler::the().read(is, "binfileCC.osb");

   MyCollector = new Collector(scene);
...
void display(void)
{
   MyCollector->prepareFrame(mgr->getWindow()->getPort(0));
   mgr->redraw();
}
