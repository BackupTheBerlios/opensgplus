...
(Int32 i, HalfedgeHandle &fhe_h, HalfedgeHandle &op)
{
   ...
   if (op == test) {                                                             // corner
      ...
      ppatch->valenz[i] = vt;  
      // one more to make later calculation easier
      test = mesh->prev_halfedge_handle(mesh->opposite_halfedge_handle(test)); 
      vt++;
      // quads: also from of prev
      p=mesh->point(mesh->from_vertex_handle(mesh->prev_halfedge_handle(test)));  // gamma
...
// helper for storing for example creases 2 and 7 (at corner 0) and so on
template<>
...
template<>
void WSDmesh2dat<OSG::Vec3f, MyPolyMesh, QUAD>::put_p_to
...
//*******
   ...
   // crease
   if (useCreases)
   ppatch->crease[8] = mesh->property(isCrease,mesh->edge_handle(fhe_h));//mesh->edge(mesh->edge_handle(fhe_h)).isCrease;
   else
    ppatch->crease[8] = 0;
...
//*******
   ...
   // crease
   if (useCreases)
   ppatch->crease[6] = mesh->property(isCrease,mesh->edge_handle(fhe_h));//mesh->edge(mesh->edge_handle(fhe_h)).isCrease;
   else
    ppatch->crease[6] = 0;
...
//*******
   ...
   // crease
   if (useCreases) {
      ppatch->crease[3] = mesh->property(isCrease,mesh->edge_handle(fhe_h));//mesh->edge(mesh->edge_handle(fhe_h)).isCrease;
   } else {
      ppatch->crease[3] = 0;
   }
   // tex
...
//*************************************************************************************
   //store all creases in separate array
   ...
   for (Int32 ll=0; ll<4; ll++) {
      ...
      //}
      //else ppatch->iscorner[ll] = 3;      //  valence = 2: corner!
   }    
