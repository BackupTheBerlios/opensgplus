...
#include "OSGWMlookupa.h"
#include <stdlib.h>
#include "OSGWMhashtab.h"
 ...
 */
static void hgrow(htab *t)  /* table */
{
    ...
    for(i = newsize >> 1; i--;)
    {
        register hitem  *thisItem, *that, **newplace;
        for(thisItem = oldtab[i]; thisItem;)
        {
            that = thisItem;
...
void hdestroy(htab *t)      /* the table */
{
    ...
    //GEOMARK_FREE((char *)t->table);
    //GEOMARK_FREE((char *)t);
    GEOMARK_FREE(t->table);
    GEOMARK_FREE(t);
}
...
/* key to find */ ub4 keyl) /* key length */
{
    hitem   *h;
...
word hadd(htab *t,
    ...
    /* key to add to hash table */
          ub4 keyl,

    /* key length */
          void *stuff)      /* stuff to associate with thisItem key */
{
    register hitem  *h, **hp;
...
void hstat(htab *t)
{
    ...
    for(i = 0; i <= t->mask; ++i)
    {
        ...
        else
        {
            ...
            {
                for(walk2 = stat; walk2->next && (walk2->next->keyl < j);
                    walk2 = walk2->next)
                    ;
