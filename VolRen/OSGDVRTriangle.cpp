...
{
  cutPoint = (GLdouble*)realloc(cutPoint,(6+additionalPerVertexAttributes)*sizeof(GLdouble));
  return cutPoint != NULL;
}

void DVRTriangle::dump() const
{
  std::cerr <<"Verts: " << vertices[0] << " " << vertices[1] << " " << vertices[2]
       <<" Neighbours: " << neighbours[0] << " " << neighbours[1] << " " << neighbours[2] << std::endl;
}
