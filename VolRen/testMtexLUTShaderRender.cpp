...
void display(void)
{
    ...
    if (newMode != curMode)
    {
        beginEditCP(shader, DVRMtexLUTShader::LutModeFieldMask);
	setField<Int8>(shader, "lutMode", newMode);
	endEditCP(shader, DVRMtexLUTShader::LutModeFieldMask);
    }
...
int setupGLUT(int *argc, char *argv[])
{
    ...
    int winid = glutCreateWindow("OpenSG DVRMtexLUTShader");
...
{  
    ...
    // Create a shader
    shader = DVRMtexLUTShader::create();
