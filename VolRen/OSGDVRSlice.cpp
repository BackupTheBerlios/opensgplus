...
Pnt3f DVRSlice::getVertex(unsigned int number)
{
  return vertexData[number];
}

unsigned int DVRSlice::getVertexCount()
{
...
Vec3f DVRSlice::getNormal()
{
  ...
  if (normal == Vec3f(0.0,0.0,0.0) && numberOfVertices >= 3){
    normal = (vertexData[0] - vertexData[1]).cross(vertexData[0] - vertexData[2]);
    normal.normalize();
  }
  return normal;
}
...
Vec3f DVRSlice::getTextureScale() const
{
  return texScale;
}

Vec3f DVRSlice::getTextureTranslate() const
{
  return texTranslate;
}

void DVRSlice::output()
{
  for (UInt32 i = 0; i < numberOfVertices; i++)
    std::cerr << "Vertex Number " << i << ": " << vertexData[i][0] << ", "<< vertexData[i][1] 
	 << ", "<< vertexData[i][2] << std::endl;

  Vec3f n = getNormal();
  std::cerr << "Normal: " << n[0] << ", " << n[1] << ", " << n[2] << std::endl;
}
