...
static void CALLBACK beginCallback(GLenum which, void *data);
static void CALLBACK endCallback(void *data);
static void CALLBACK vertexCallback(GLvoid* vertexData, void *data);
static void CALLBACK vertexCombineCallback(GLdouble*, GLdouble**, GLfloat*, GLdouble**, void *data);
...
void DVRClipper::setReferencePlane(const Plane &rP)
{
    FDEBUG(("pre DVRVolume::setRefPlane - %f %f %f : %f\n",
            rP.getNormal()[0], rP.getNormal()[1], rP.getNormal()[2], 
            rP.getDistanceFromOrigin()));

    refPlane.set(rP.getNormal(),rP.getDistanceFromOrigin());

    FDEBUG(("post DVRVolume::setRefPlane - %f %f %f : %f\n",
            refPlane.getNormal()[0], refPlane.getNormal()[1], 
            refPlane.getNormal()[2], refPlane.getDistanceFromOrigin()));
}
...
bool DVRClipper::setNumAddPerVertexAttr(DVRVolume *volume, UInt32 additionalPerVertexAttributes)
{
    numAddPerVertexAttr = additionalPerVertexAttributes;

    sliceVertexData = (GLdouble*)realloc(sliceVertexData,6*(6+additionalPerVertexAttributes)*sizeof(GLdouble));
...
void DVRClipper::initialize(DVRVolume *volume)
{
    ...
    if(myTess == NULL){
        ...
        gluTessCallback(myTess, GLU_TESS_COMBINE_DATA, (GLvoid (*)())vertexCombineCallback);
        gluTessCallback(myTess, GLU_TESS_VERTEX_DATA, (GLvoid (*)())vertexCallback);
        gluTessCallback(myTess, GLU_TESS_BEGIN_DATA, (GLvoid (*)())beginCallback);
        gluTessCallback(myTess, GLU_TESS_END_DATA, (GLvoid (*)())endCallback);
        gluTessCallback(myTess, GLU_TESS_ERROR, (GLvoid (*)())errorCallback);
...
void DVRClipper::reset(DVRVolume *volume)
{
    ...
    if(clipObjects->getClipMode() == DVRClipObjects::Intersection)
        gluTessProperty(myTess, GLU_TESS_WINDING_RULE, GLU_TESS_WINDING_ABS_GEQ_TWO);
    else
        gluTessProperty(myTess, GLU_TESS_WINDING_RULE, GLU_TESS_WINDING_POSITIVE);
			   ...
			   DVRRenderSlice &clippedSlice)
{
    ...
    if(clipObjects == NullFC){
        ...
        for(UInt32 i = 0; i < unclippedSlice.getVertexCount(); i++){
            ...
            sliceVertexData[idx] = unclippedSlice.getVertex(i).getValues()[0];
            sliceVertexData[idx+1] = unclippedSlice.getVertex(i).getValues()[1];
            sliceVertexData[idx+2] = unclippedSlice.getVertex(i).getValues()[2];
            // set (standard) texture coordinates
            sliceVertexData[idx+3] = texScale[0]*unclippedSlice.getVertex(i).getValues()[0]+texTranslate[0];
            sliceVertexData[idx+4] = texScale[1]*unclippedSlice.getVertex(i).getValues()[1]+texTranslate[1];
            sliceVertexData[idx+5] = texScale[2]*unclippedSlice.getVertex(i).getValues()[2]+texTranslate[2];      
    ...
    if(clipObjects->getDoContours()){
        ...
        for (UInt32 i = 0; i < unclippedSlice.getVertexCount(); i++){      
            glColor3f(col % 3 == 0 ? 1.0 : 0.0, col % 3 == 1 ? 1.0 : 0.0, col % 3 == 2 ? 1.0 : 0.0);
            col++;
        ...
        bool clipAwayOutside = clipObjects->getClipMode() == DVRClipObjects::Difference;
        ...
        for (UInt32 i = 0; i < clipObjects->count(); i++){
            ...
            // compute the contours of the triangles intersecting the current slice
            ...
            if (!contours.empty()){
                ...
                for(contoursIt = contours.begin(); contoursIt != contours.end(); contoursIt++){ 
    ...
    // tesselate and render the clipped slices
        ...
        gluTessNormal(myTess, slicingNormal[0], slicingNormal[1], slicingNormal[2]);    
        ...
        for (UInt32 i = 0; i < unclippedSlice.getVertexCount(); i++){       
            ...
            sliceVertexData[idx+4] = texScale[1]*unclippedSlice.getVertex(i).getValues()[1]+texTranslate[1];
            sliceVertexData[idx+5] = texScale[2]*unclippedSlice.getVertex(i).getValues()[2]+texTranslate[2];
            gluTessVertex(myTess, &sliceVertexData[idx], &sliceVertexData[idx]);
        }
                ...
                if (!contours.empty()){
                    ...
                    for(contoursIt = contours.begin(); contoursIt != contours.end(); contoursIt++){ 
                        ...
                        do{	    
                            ...
                            // set (standard) texture coordinates
                            current->cutPoint[3] = texScale[0]*current->cutPoint[0]+texTranslate[0];
                            current->cutPoint[4] = texScale[1]*current->cutPoint[1]+texTranslate[1];
                            current->cutPoint[5] = texScale[2]*current->cutPoint[2]+texTranslate[2];
                            if(!current->cutPoint)
                                std::cerr<<"WTF: cutPoint is NULL"<<std::endl;
                            gluTessVertex(myTess, current->cutPoint, current->cutPoint);
                            current = current->contourNeighbour;
...
{
    SFATAL << "Error " << gluErrorString(errorCode) << " occured during tesselation!" << std::endl;
}

void CALLBACK 
beginCallback(GLenum which, void *data)
{
...
void CALLBACK 
vertexCallback(GLvoid *vertexData, void *data)
{
...
void CALLBACK 
vertexCombineCallback(GLdouble *coords, GLdouble *vertexData[4], GLfloat weight[4], GLdouble **dataOut, void *data)
{
    DVRRenderSlice *sliceData = (DVRRenderSlice*)data;
    ...
    if(sliceData->maxVertexCombineData <= sliceData->numVertexCombineData){
        ...
        sliceData->vertexCombineData 
            = (GLdouble *)realloc(sliceData->vertexCombineData,
                                  (6+sliceData->numPerVertexData)*sliceData->maxVertexCombineData*sizeof(GLdouble));
        FDEBUG(("realloc vd %i\n",sliceData->numVertexCombineData));
    }

    GLdouble *vertex = &(sliceData->vertexCombineData[(6+sliceData->numPerVertexData)*sliceData->numVertexCombineData]);
    sliceData->numVertexCombineData++;
